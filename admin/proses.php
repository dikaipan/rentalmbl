...
class ClassMobil
{
  ...
  {
    $this->koneksi = mysqli_connect("localhost","root","","rentalmbl");
  }
  ...
  public function selectMobilHarga(){
	...
	//$query = mysqli_query($this->koneksi,"SELECT b.*, b.id as idMobil,d.merek as namaMerek FROM  mobil b, kategori d 
 // WHERE b.merek=d.id");
    while ($r = mysqli_fetch_array($query)) {
      $data[] = $r;
    }
  ...
  public function insertCustomer($namalengkap, $username, $alamat,$telepon,$password,$noktp)
  {
    ...
    if ($query) {
      ...
      <script>
        ...
        if (cnf) {location.href='../rentalmbl/index.php'}
      </script>
      <?php
    }else{
  ...
  public function selectCustomer($username, $password)
  {
    ...
    if ($data > 0){
      ...
      echo "<meta http-equiv='refresh' content='0; url=../rentalmbl/index.php'>";
  }else{
    ?>
...
<?php
  ...
  public function cariNamaMobil($nama){
      $query = mysqli_query($this->koneksi,"SELECT a.*, b.*, b.id as idMobil, d.merek as namaMerek 
                                            FROM harga a, mobil b, kategori d 
                                            WHERE a.mobil=b.id AND b.merek=d.id 
                                            AND b.nama='$nama'");
    while ($r = mysqli_fetch_array($query)) {
    $data[] = $r;
    }
  ...
  public function reservasi($idmobil,$userid,$kode,$harga,$tglawal,$durasi,$status,$totBayar){
	  //$totalBayar=$harga*$durasi;	  //TOTAL BAYAR SEWA MOBIL
	  //print_r($totBayar);exit;
	$query = mysqli_query($this->koneksi,"INSERT INTO reservasi(id_mobil,user_id,nobooking,harga,tgl_pinjam,durasi,status,totalbayar) 
	VALUES ('$idmobil','$userid','$kode','$harga','$tglawal','$durasi','$status','$totBayar')");
	if ($query) {
      echo "
      <script>
        var cnf = confirm('Pesanan anda sedang di proses, silahkan melakukan pembayaran');
        if (cnf) {location.href='../rentalmbl/bayar.php?kodebooking=$kode'}
      </script>
      ";
    }else{
  ...
  public function jmlbayar(){
  $query = mysqli_query($this->koneksi,"select totalbayar from reservasi");
